package kw.ecs.cn.pwcspcmprsttus.service.impl;

import java.util.List;
import java.util.Map;

import javax.annotation.Resource;

import org.apache.log4j.Logger;
import org.springframework.stereotype.Service;

import kw.ecs.cn.pwcspcmprsttus.service.PwcspCmprSttusService;
import kw.ecs.cn.pwcspcmprsttus.vo.PwcspCmprSttusVo;
import kw.ecs.dao.EcsCommonDao;
import kw.fw.defaults.DefaultServiceImpl;

/**
 * PwcspCmprSttusServiceImpl 서비스 클래스
 * 
 * @ClassName PwcspCmprSttusServiceImpl.java
 * @Description 소비 - 사업장 소비전력 비교현황 클래스
 * @Modification_Information
 * 
 *                           <pre>
 *	수정일					수정자						수정내용
 * ------------		-----------------		-------------------------------
 * 2023.03.15		Soltech 김유빈			최초생성
 *                           </pre>
 * 
 * @author Soltech 김유빈
 * @since 2023. 03. 15
 * @version 1.0
 * @see
 * 
 *      <pre>
 * Copyright (C) 2023 by Unlimited K-water, All right reserved.
 *      </pre>
 */
@SuppressWarnings("rawtypes")
@Service("pwcspCmprSttusService")
public class PwcspCmprSttusServiceImpl extends DefaultServiceImpl implements PwcspCmprSttusService {
	protected final static Logger logger = Logger.getLogger(PwcspCmprSttusServiceImpl.class);

	/** EcsCommonDao 적용 */
	@Resource(name = "ecsCommonDao")
	private EcsCommonDao ecsCommonDao;

	/**
	 * 소비의 소비전력 비교현황 목록을 조회한다.
	 * 
	 * @param @PwcspCmprSttusVo
	 * @return list 데이터 맵 리스트 객체
	 * @throws Exception
	 */
	@Override
	public List<Map> retrievePwcspCmprSttusList(PwcspCmprSttusVo pwcspCmprSttusVo) throws Exception {
		return ecsCommonDao.selectList("PWCSP_CMPR_STTUS_Dao_retrievePwcspCmprSttusList", pwcspCmprSttusVo);
	}

	/**
	 * 소비의 소비전력 비교현황 기준 차트를 조회한다.
	 * 
	 * @param @PwcspCmprSttusVo
	 * @return list 데이터 맵 리스트 객체
	 * @throws Exception
	 */
	@Override
	public List<Map> retrievePwcspCmprSttusChart(PwcspCmprSttusVo pwcspCmprSttusVo) throws Exception {
		return ecsCommonDao.selectList("PWCSP_CMPR_STTUS_Dao_retrievePwcspCmprSttusChart", pwcspCmprSttusVo);
	}

	/**
	 * 소비의 소비전력 비교현황 비교 차트를 조회한다.
	 * 
	 * @param @PwcspCmprSttusVo
	 * @return list 데이터 맵 리스트 객체
	 * @throws Exception
	 */
	public List<Map> retrievePwcspCmprSttusChartTarget(PwcspCmprSttusVo pwcspCmprSttusVo) throws Exception {
		return ecsCommonDao.selectList("PWCSP_CMPR_STTUS_Dao_retrievePwcspCmprSttusChartTarget", pwcspCmprSttusVo);
	}
}